...
function searchRigSlot(slotNum, query) {
  ...
  const noneRow = `<div class="px-1.5 py-1 hover:bg-[#2a343c] cursor-pointer text-slate-400 border-b border-[#2a343c]/40" onmousedown="selectRigForSlot(${slotNum}, 0, '')">— None —</div>`;
  const matchRows = matches.length > 0
    ? matches.map(r => `<div class="px-1.5 py-1 hover:bg-[#2a343c] cursor-pointer border-b border-[#2a343c]/20" onmousedown="selectRigForSlot(${slotNum}, ${r.typeId}, '${window.esc(r.name)}')">${window.esc(r.name)}</div>`).join('')
...
function populateBlueprintLocationDropdown() {
  ...
  filterSelect.innerHTML = `
    <option value="all" style="color: #38bdf8; background-color: #0f1418; font-weight: bold;">All Locations</option>
    <option value="industry_system" style="color: #38bdf8; background-color: #0f1418; font-weight: bold;">Current System Only (${currentSystemName})</option>
  `;
  const sagNameMap = {
  ...
  for (const [locId, data] of Object.entries(locCounts)) {
    ...
    mainOpt.style.backgroundColor = '#0f1418';
    ...
    for (const [sagFlag, sagData] of Object.entries(data.corpDivisions)) {
      ...
      sagOpt.style.backgroundColor = '#07090b';
    ...
    for (const [cId, cData] of Object.entries(data.containers)) {
      ...
      containerOpt.style.backgroundColor = '#07090b';
...
function renderBlueprintBrowserList(list, stackEnabled, sortByProfit) {
  ...
  const renderRow = (bp, showStationLabel) => {
    ...
    return `
      <div class="bg-[#171f26] border border-[#2a343c] hover:border-cyan-500 rounded p-2.5 transition space-y-2">
        <div class="flex items-center gap-2 min-w-0">
          <img src="https://images.evetech.net/types/${bp.type_id}/${bpImageVariant}?size=32" class="w-8 h-8 rounded border border-slate-700 bg-[#07090b] flex-shrink-0" loading="lazy" title="${isOriginal ? 'Blueprint Original (BPO)' : 'Blueprint Copy (BPC)'}">
...
async function searchHomeMarket(query) {
  ...
  resultsEl.innerHTML = matches.map(m => `
    <div class="px-2 py-1.5 hover:bg-[#2a343c] cursor-pointer border-b border-[#2a343c]/20" onmousedown="selectHomeMarket(${m.stationId}, '${window.esc(m.stationName)}')">
...
function renderTrackedMarketsList() {
  ...
  listEl.innerHTML = markets.map(m => `
    <div class="flex items-center justify-between bg-[#07090b] border border-[#2a343c] rounded px-2 py-1 text-[10px]">
...
async function searchAddMarket(query) {
  ...
  resultsEl.innerHTML = matches.map(m => `
    <div class="px-2 py-1.5 hover:bg-[#2a343c] cursor-pointer border-b border-[#2a343c]/20" onmousedown="confirmAddMarket(${m.stationId}, '${window.esc(m.stationName)}')">
      ${window.esc(m.stationName)}
    </div>
  `).join('');
}
...
async function openMarketComparison(e, typeId, itemName) {
  ...
  modal.innerHTML = `
    <div class="bg-[#0f1418] border border-cyan-500/80 rounded-lg p-5 w-full max-w-2xl shadow-2xl text-xs mono">
      <div class="flex justify-between items-center border-b border-[#2a343c] pb-3 mb-3">
  ...
  const rows = results.map(r => `
    <tr class="border-b border-[#2a343c]/40">
  ...
  bodyEl.innerHTML = `
    <table class="w-full text-left border-collapse">
      <thead>
        <tr class="text-slate-400 border-b border-[#2a343c] uppercase text-[10px] font-bold">
    ...
    if (searchResults) {
      searchResults.innerHTML = hits.map(item => {
        ...
        const displayIconId = isBp
          ...
          : item.id;
        return `
        <div class="px-3 py-2 hover:bg-[#2a343c] cursor-pointer flex items-center space-x-3 text-xs border-b border-[#2a343c]/40"
  ...
  systemSearchInput.addEventListener('input', async () => {
    ...
    if (systemSearchResults) {
      systemSearchResults.innerHTML = hits.map(sys => `
        <div class="px-3 py-1.5 hover:bg-[#2a343c] cursor-pointer text-xs font-bold text-cyan-300 border-b border-[#2a343c]/40 mono"
...
function createNodeCard(node) {
  ...
  let cardStyle = 'bg-[#161d23] border border-[#2a343c] w-64';
  if (isRoot) cardStyle = 'bg-[#1c252c] border-2 border-cyan-500 w-80';
  else if (!node.isBuildingSelf) cardStyle = 'bg-[#12181d] border border-slate-600/80 w-64';
  else if (node.isReaction) cardStyle = 'bg-[#161b26] border border-cyan-700/60 w-64';
  else if (node.batchYield > 1) cardStyle = 'bg-[#211c14] border border-amber-700/60 w-64';
  ...
  if (isRoot) {
    ...
    sellStrategyUI = `
      <div class="mb-2 p-1.5 bg-[#07090b] rounded border border-cyan-500/40 space-y-1.5" onclick="event.stopPropagation()">
        <div class="flex justify-between items-center text-xs mono">
          <span class="text-cyan-300 font-bold">Sell Channel:</span>
          <select id="card-sell-strategy" onchange="syncSellStrategy(event)" class="bg-[#0f1418] text-white rounded p-0.5 border border-[#2a343c] text-xs outline-none">
          ...
          <div class="flex flex-col text-xs mono">
            <div class="flex justify-between items-center">
              ...
              <div class="flex items-center space-x-1">
                <input type="number" id="card-custom-price" value="${curCustomPrice}" placeholder="Unit Price" onchange="syncCustomPrice(event)" class="w-24 bg-[#0f1418] border border-[#2a343c] text-center text-green-400 font-bold rounded p-0.5 outline-none text-xs">
  ...
  card.innerHTML = `
    ...
    <div class="flex items-center space-x-3 border-b border-[#2a343c] pb-2.5 mb-2.5">
      <img src="${iconUrl}" class="w-10 h-10 border border-slate-700 bg-[#07090b] flex-shrink-0" style="clip-path: polygon(0 4px, 4px 0, 100% 0, 100% 100%, 0 100%);" onerror="this.onerror=null; this.src='https://images.evetech.net/types/${productTypeId}/icon?size=64';">
          ...
          <div class="flex items-center space-x-1 flex-shrink-0 ml-1">
            ${isRoot ? `
              <div class="relative group inline-block" onclick="event.stopPropagation()">
                ...
                <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block bg-[#07090b] border border-cyan-500 text-white text-xs p-2 rounded shadow-2xl z-[999] whitespace-nowrap mono pointer-events-none">
                  <div class="text-cyan-300 font-bold border-b border-[#2a343c] pb-1 mb-1">Estimated Item Value (EIV)</div>
            ...
            ${node.children && node.children.length > 0 ? `
              <button onclick="toggleNodeCollapse(event, ${node.instanceId})" class="hud-badge text-xs ${window.collapsedInstanceIds.has(node.instanceId) ? 'bg-cyan-700 hover:bg-cyan-600 text-white' : 'bg-[#2a343c] hover:bg-slate-600 text-slate-300'} px-1.5 py-0.5 mono transition" title="${window.collapsedInstanceIds.has(node.instanceId) ? 'Expand: show inputs again' : 'Collapse: hide inputs'}">
            ...
            ` : `
              <button onclick="isolateComponent(event, ${node.instanceId})" class="hud-badge text-xs bg-[#2a343c] hover:bg-cyan-600 text-cyan-200 px-1.5 py-0.5 mono transition">🔍 Isolate</button>
            `}
          </div>
        </div>
    ...
    <div class="space-y-2.5">
      ${isRoot ? `
        <div class="p-1.5 bg-[#07090b] hud-panel border border-cyan-500/40 flex items-center justify-between text-sm mono" onclick="event.stopPropagation()">
          <span class="text-slate-300 font-bold">Runs:</span>
          <div class="flex items-center space-x-1">
            <input type="number" id="card-bp-runs" value="${node.runsNeeded}" min="1" max="1000000" onchange="syncCardRunsToGlobal(event)" onkeydown="if(event.key==='Enter') this.blur()" class="w-16 bg-[#0f1418] border border-cyan-500/60 text-center text-cyan-300 font-bold rounded p-0.5 outline-none">
      ...
      ${(!isRoot && node.isManufacturable) || (!isRoot && (!node.isBuildingSelf || !node.children || node.children.length === 0)) || (node.isBuildingSelf && node.isManufacturable && !node.isReaction) ? `
        <div class="bg-[#07090b] hud-panel border border-[#2a343c]/60 divide-y divide-[#2a343c]/40" onclick="event.stopPropagation()">
          ${!isRoot && node.isManufacturable ? `
            <div class="flex items-center justify-between px-2 py-1.5 text-xs mono">
              ...
              <div class="flex space-x-1">
                <button onclick="toggleBuildSelf(event, ${node.typeId})" class="hud-badge px-2 py-0.5 font-bold transition ${node.isBuildingSelf ? 'bg-green-600 text-white' : 'bg-[#2a343c] text-slate-400 hover:text-white'}">🔨 Build</button>
                <button onclick="toggleBuildSelf(event, ${node.typeId})" class="hud-badge px-2 py-0.5 font-bold transition ${!node.isBuildingSelf ? 'bg-cyan-600 text-black' : 'bg-[#2a343c] text-slate-400 hover:text-white'}">🛒 Buy</button>
              </div>
            </div>
          ` : ''}
          ${!isRoot && (!node.isBuildingSelf || !node.children || node.children.length === 0) ? `
            <div class="flex items-center justify-between px-2 py-1.5 text-xs mono">
              ...
              <div class="flex space-x-1">
                <button onclick="setComponentBuyMode(event, ${node.typeId}, 'sell')" class="hud-badge px-1.5 py-0.5 font-bold transition ${currentBuyStrategy === 'sell' ? 'bg-cyan-600 text-black' : 'bg-[#2a343c] text-slate-400 hover:text-white'}" title="Instant Buy off Sell Orders">⚡ Sell</button>
                <button onclick="setComponentBuyMode(event, ${node.typeId}, 'buy')" class="hud-badge px-1.5 py-0.5 font-bold transition ${currentBuyStrategy === 'buy' ? 'bg-cyan-600 text-white' : 'bg-[#2a343c] text-slate-400 hover:text-white'}" title="Order Placing via Buy Orders">📜 Buy</button>
              </div>
            </div>
          ` : ''}
          ${node.isBuildingSelf && node.isManufacturable && !node.isReaction ? `
            <div class="flex items-center justify-between px-2 py-1.5 text-xs mono" onmousedown="event.stopPropagation()">
              ...
              <div class="flex items-center space-x-1">
                <input type="number" id="card-me-${node.instanceId}" min="0" max="10" value="${node.customME}" onchange="onCardMEChange(event, ${node.typeId}, ${node.instanceId})" class="w-10 bg-[#0f1418] border border-[#2a343c] text-center text-cyan-400 font-bold rounded p-0.5 outline-none focus:border-cyan-500">
                <span class="text-slate-500">%</span>
                <input type="number" id="card-te-${node.instanceId}" min="0" max="20" value="${node.customTE}" onchange="onCardTEChange(event, ${node.typeId}, ${node.instanceId})" class="w-10 bg-[#0f1418] border border-[#2a343c] text-center text-cyan-400 font-bold rounded p-0.5 outline-none focus:border-cyan-500">
                <span class="text-slate-500">%</span>
              </div>
            </div>
          ` : ''}
      ...
      <div class="text-sm mono space-y-1 hud-divider pt-2">
        <div class="flex justify-between items-center font-semibold">
          ...
          <div class="flex items-center gap-1.5">
            ...
            <button onclick="openMarketComparison(event, ${productTypeId}, '${window.esc(node.productName || node.name)}')" class="hud-badge text-xs bg-[#2a343c] hover:bg-cyan-600 text-cyan-200 px-1.5 py-0.5 transition flex items-center gap-1" title="Compare price and trade volume across your tracked markets">⇄ Compare</button>
          </div>
        </div>
...
function renderIsolatedDiagram() {
  ...
  if (!isolatedNode.isBuildingSelf || isolatedNode.children.length === 0) {
    inputCol.innerHTML = `<div class="bg-[#0f1418] border border-[#2a343c] p-3 text-xs text-slate-400 mono rounded">${!isolatedNode.isBuildingSelf ? 'Purchased off Market (No decomposed inputs)' : 'No inputs (Base Material)'}</div>`;
  } else {
  ...
  if (parentNode) {
    ...
    outputCol.innerHTML = `<div class="bg-[#0f1418] border border-cyan-500/50 p-3 text-xs text-cyan-300 font-bold mono rounded">Final Target Output</div>`;
  }
...
function renderBillOfMaterials(rootNode, brokerFee = 0) {
  ...
  bomItems.forEach(item => {
    ...
    row.className = 'bg-[#0f1418] hud-panel border border-[#2a343c] hover:border-cyan-500 hover:bg-[#101d2a] p-2 flex items-center justify-between cursor-pointer transition shadow-sm';
    ...
    row.innerHTML = `
      <div class="flex items-center space-x-2.5 min-w-0">
        <img src="https://images.evetech.net/types/${item.typeId}/icon?size=32" class="w-7 h-7 rounded border border-slate-700 bg-[#07090b] flex-shrink-0" loading="lazy" onerror="this.onerror=null; this.src='https://images.evetech.net/types/${item.typeId}/render?size=32';">
        <div class="min-w-0 flex-1">
