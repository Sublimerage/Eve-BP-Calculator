...
// --- Structure Rig Catalog ---
// Rigs only affect certain item categories (Ammunition, Ships, Drones/Fighters, Modules, Structures),
// not everything uniformly - unlike a flat % bonus. categoryIds are verified against EVE Ref's category
// list. NOTE: this app can't yet distinguish ship hull-size classes (Frigate vs Cruiser vs Battleship),
// so all ships are grouped into one "Ships (Any)" bucket - a rig restricted to e.g. only Small Ships
// in-game will be applied to any ship here. This is a known simplification, not a data bug.
const RIG_CATALOG = [
  { key: 'ammo', label: 'Ammunition & Charges', categoryIds: [8] },
  { key: 'ship', label: 'Ships (Any)', categoryIds: [6] },
  { key: 'drone', label: 'Drones & Fighters', categoryIds: [18, 87] },
  { key: 'module', label: 'Equipment & Modules', categoryIds: [7, 66] },
  { key: 'structure', label: 'Structures & Components', categoryIds: [65, 4, 17] }
];
window.RIG_CATALOG = RIG_CATALOG;
...
const RIG_TIERS = [
  { key: 'T1', label: 'I', meBase: 2.0, teBase: 20.0 },
  { key: 'T2', label: 'II', meBase: 2.4, teBase: 24.0 }
];
window.RIG_TIERS = RIG_TIERS;
...
// Resolves which rig category bucket a product belongs to, using the category data generated by
// generate_db.py (window.EVE_CATEGORIES). Returns null if unknown (no rig will match it).
function getRigCategoryKeyForTypeId(typeId) {
  const catId = window.EVE_CATEGORIES ? window.EVE_CATEGORIES[typeId] : undefined;
  if (catId === undefined || catId === null) return null;
  for (const bucket of RIG_CATALOG) {
    if (bucket.categoryIds.includes(catId)) return bucket.key;
  }
  return null;
}
window.getRigCategoryKeyForTypeId = getRigCategoryKeyForTypeId;

// Reads the 3 rig slot selections and returns the effective % bonus (already including the security
// multiplier) for the given product typeId and bonus type ('ME' or 'TE'). Only one rig "of the same
// type" can be fitted in-game, so matching slots take the max rather than summing.
function getEffectiveRigBonusForTypeId(typeId, bonusType) {
  const catKey = getRigCategoryKeyForTypeId(typeId);
  if (!catKey) return 0;
  const secMult = getSecurityMultiplier();
  let best = 0;
  for (let slot = 1; slot <= 3; slot++) {
    const val = localStorage.getItem(`eve_rig_slot_${slot}`) || 'none';
    if (val === 'none') continue;
    const parts = val.split('|');
    if (parts.length !== 3) continue;
    const [cat, type, tierKey] = parts;
    if (cat !== catKey || type !== bonusType) continue;
    const tierData = RIG_TIERS.find(t => t.key === tierKey);
    if (!tierData) continue;
    const base = bonusType === 'ME' ? tierData.meBase : tierData.teBase;
...
// Builds the <option> list (with category optgroups) shared by all 3 rig slot dropdowns.
function buildRigSlotOptionsHTML() {
  let html = '<option value="none">None</option>';
  RIG_CATALOG.forEach(bucket => {
    html += `<optgroup label="${bucket.label}">`;
    RIG_TIERS.forEach(tier => {
      html += `<option value="${bucket.key}|ME|${tier.key}">${bucket.label} - Material Eff. ${tier.label}</option>`;
      html += `<option value="${bucket.key}|TE|${tier.key}">${bucket.label} - Time Eff. ${tier.label}</option>`;
    });
    html += `</optgroup>`;
  });
  return html;
}
window.buildRigSlotOptionsHTML = buildRigSlotOptionsHTML;
